...
class Tank_auth
{
	...
	/**
	 * Check if user owns the site by ID.
	 *
	 * @param	int
	 * @param   int
	 * @return	bool
	...
	function auth_init( $auth_required = false, $referrer = false ) {
		$data = array();
		$data['is_logged_in'] = $this->is_logged_in();
		$data['ssl'] = $this->ci->config->item('ssl');
		
		if (!$data['is_logged_in'] && !$auth_required) {
			$data['user_id'] = $data['username'] = $data['userinfo'] = $data['userCompany'] = 0;
			
		} else if (!$data['is_logged_in']) {
			redirect('/auth/login/'.str_replace("/","-",$referrer));
			
		} else {
			$data['user_id']	= $this->get_user_id();
			$data['username']	= $this->get_username();
			$data['userinfo']   = $this->get_userinfo( $data['user_id'] );
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'user_level' => ''));
	...
	function get_userinfo( $id ) {
		return $this->ci->users->get_user_by_id( $id );
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $fname, $lname, $email_activation)
	{
